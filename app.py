...
def get_article_ranking_comparison(property_id, days):
    ...
    try:
        ...
        # ここに来る＝APIが「organicGoogleSearchQueryなんて知らない」と言っている可能性大
        # なので、キーワード取得を諦めて「流入元」だけを取りに行く
        ...
        try:
            ...
            return pd.DataFrame() # 万策尽きたら空を返す
...
def run_diagnostic(property_id):
    ...
    try:
        ...
        if len(valid_kw_sample) > 0:
            ...
            st.markdown(f"**検出されたキーワード:** `{', '.join(valid_kw_sample[:5])}`...")
        else:
        ...
        if "not a valid dimension" in err_msg or "organicGoogleSearchQuery" in err_msg:
            st.error(f"""
            **【重要】Search Console機能が無効です**
            エラー内容: `organicGoogleSearchQuery is not a valid dimension`
            
            ID `{property_id}` は合っているとのことですが、Google API側が
            「このプロパティには検索キーワード機能がない」と判定しています。
            
            **考えられる理由:**
            1. 連携してからAPIに反映されるまで、稀に数日〜1週間かかることがあります。
            2. GA4上でリンクを作成したユーザーと、APIキー（サービスアカウント）の権限の不一致。
            
            **対策:**
            このアプリは自動的に「流入元モード」で動作し続けます。
            あと数日待って、API側の反映を待ってください。
            """)
        else:
